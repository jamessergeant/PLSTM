...
def multiPLSTM(input, batch_size, lens, n_layers, units_p_layer, n_input, initial_states,lstm_type='PLSTM'):
    """
...
class ModPLSTMCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with vs.variable_scope(scope or type(self).__name__,
            ...
            # s_broadcast = tf.reduce_sum(s_broadcast,axis=1)
            #
            # tau_broadcast = tf.expand_dims(tau_broadcast, dim=0)
            # r_on_broadcast = tf.expand_dims(r_on_broadcast, dim=0)
            # s_broadcast = tf.expand_dims(s_broadcast, dim=0)

            r_on_broadcast = tf.abs(r_on_broadcast)
            tau_broadcast = tf.abs(tau_broadcast)
            times = tf.tile(times, [1, self._num_units])

            # r_on_broadcast = tf.Print(r_on_broadcast,[tf.shape(r_on_broadcast)],'r_on: ')
            # tau_broadcast = tf.Print(tau_broadcast,[tf.shape(tau_broadcast)],'tau: ')
            # s_broadcast = tf.Print(s_broadcast,[tf.shape(s_broadcast)],'s: ')
            # times = tf.Print(times,[tf.shape(times)],'times: ')

            # calculate kronos gate
            phi = tf.div(tf.mod(tf.mod(times - s_broadcast, tau_broadcast) + tau_broadcast, tau_broadcast),
                         tau_broadcast)
            # phi = tf.Print(phi,[tf.shape(phi)],'phi: ')

            is_up = tf.less(phi, (r_on_broadcast * 0.5))
            # is_up = tf.Print(is_up,[tf.shape(is_up)],'is_up: ')

            is_down = tf.logical_and(tf.less(phi, r_on_broadcast), tf.logical_not(is_up))
            # is_down = tf.Print(is_down,[tf.shape(is_down)],'is_down: ')

            k = tf.select(is_up, phi / (r_on_broadcast * 0.5),
                              tf.select(is_down, 2. - 2. * (phi / r_on_broadcast), self.alpha * phi))

            # k = tf.Print(k,[tf.shape(k)],'k: ')
            ...
            # c_prev = tf.Print(c_prev,[tf.shape(c_prev)],'c_prev: ')
            # c = tf.Print(c,[tf.shape(k * c)],'k * c: ')
            # c_prev = tf.Print(c_prev,[tf.shape((1. - k) * c_prev)],'(1. - k) * c_prev: ')
            # m = tf.Print(m,[tf.shape(m)],'m: ')
            # m_prev = tf.Print(m_prev,[tf.shape(m_prev)],'m_prev: ')
            # m = tf.Print(m,[tf.shape(k * m)],'k * m: ')
            # m_prev = tf.Print(m_prev,[tf.shape((1. - k) * m_prev)],'(1. - k) * m_prev: ')
